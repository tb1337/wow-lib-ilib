...
local function AutoHideTimerFrame_OnUpdate_OLD(self, elapsed)
...
local function AutoHideTimerFrame_CheckFrames(t)
	local r = false
...
local function AutoHideTimerFrame_OnUpdate(self, elapsed)
	self.checkElapsed = self.checkElapsed + elapsed
	if self.checkElapsed > 0.1 then
		if self.parent:IsMouseOver() or (self.alternateFrame and self.alternateFrame:IsMouseOver())
		 or (self.alternateFrames and AutoHideTimerFrame_CheckFrames(self.alternateFrames)) then
				self.elapsed = 0
		else
			self.elapsed = self.elapsed + self.checkElapsed
			if self.elapsed >= self.delay then
...
function tipPrototype:SetAutoHideDelayExt(delay, ...)
	local timerFrame = self.autoHideTimerFrame
	delay = tonumber(delay) or 0
	...
	if delay > 0 then
		...
		timerFrame.elapsed = 0
		timerFrame.delay = delay
		timerFrame.alternateFrames = select("#", ...) > 0 and {...} or nil -- bind 
		timerFrame:Show()
	elseif timerFrame then
		self.autoHideTimerFrame = nil
		timerFrame.alternateFrames = nil
		timerFrame:SetScript("OnUpdate", nil)
		ReleaseFrame(timerFrame)
	end
end
